...
impl Resources {
    pub fn validate(self) -> Result<Self, BudgetError> {
        for (name, value) in [
            ("geometry_mb", self.geometry_mb), ("textures_mb", self.textures_mb),
            ("framebuffer_mb", self.framebuffer_mb),
            ("vegetation_overdraw", self.vegetation_overdraw), ("cpu_ms", self.cpu_ms),
            ("gpu_upload_ms", self.gpu_upload_ms), ("cache_mb", self.cache_mb),
        ] {
        ...
        macro_rules! check {
            ($field:ident) => {
                if (self.$field as f64) > (limit.$field as f64) {
                    result.push(Exceedance { resource: stringify!($field).to_owned(), requested: self.$field as f64, limit: limit.$field as f64 });
                }
            };
        }
        check!(triangles); check!(instances); check!(draw_calls); check!(geometry_mb);
        check!(textures_mb); check!(framebuffer_mb); check!(materials);
        check!(vegetation_overdraw); check!(cpu_ms); check!(gpu_upload_ms); check!(cache_mb);
        result
    }
...
impl AddAssign for Resources {
    fn add_assign(&mut self, rhs: Self) {
        ...
        self.geometry_mb += rhs.geometry_mb; self.textures_mb += rhs.textures_mb;
        self.framebuffer_mb += rhs.framebuffer_mb; self.materials = self.materials.saturating_add(rhs.materials);
        self.vegetation_overdraw += rhs.vegetation_overdraw; self.cpu_ms += rhs.cpu_ms;
        self.gpu_upload_ms += rhs.gpu_upload_ms; self.cache_mb += rhs.cache_mb;
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum Decision { Accept, Degrade, Split, Defer, Confirm, Reject }
...
pub fn evaluate(requested: Resources, available: Resources) -> Result<Evaluation, BudgetError> {
    ...
    let splittable = exceedances.iter().all(|item| matches!(item.resource.as_str(),
        "triangles" | "instances" | "draw_calls" | "geometry_mb" | "textures_mb" | "cache_mb"));
    let decision = if exceedances.is_empty() { Decision::Accept }
        else if splittable { Decision::Split }
        else { Decision::Reject };
    Ok(Evaluation { requested, available, decision, exceedances })
}
...
mod tests {
    ...
    fn excedente_geometrico_pode_dividir() {
        let req = Resources { triangles: 20, ..Default::default() };
        let lim = Resources { triangles: 10, ..Default::default() };
