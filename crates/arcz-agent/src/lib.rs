//! Provider-agnostic agent plan for ARCZ.
//!
//! LLMs may propose actions; only validated tool calls mutate a project. This
//! keeps natural-language/photo authoring powerful without granting a model raw
//! filesystem, shell or scene ownership.
...
pub fn validate_plan(plan: &ActionPlan) -> Result<(), AgentPlanError> {
    ...
    for action in &plan.actions {
        ...
        if !ids.insert(action.id.clone()) {
            return Err(AgentPlanError::DuplicateAction(action.id.clone()));
        }
        if action.request.project_id != plan.intent.project_id
            || action.request.expected_revision < plan.intent.expected_revision
        {
...
pub enum AgentPlanError {
    #[error("agent intent and project are required")]
    MissingIntent,
    #[error("action id and plugin id are required")]
    MissingActionIdentity,
    #[error("duplicate action {0}")]
    DuplicateAction(String),
    #[error("project/revision mismatch in action {0}")]
    RevisionOrProjectMismatch(String),
    #[error("unsafe mutation without dry-run or approval: {0}")]
    UnsafeMutation(String),
    #[error("invalid dependency {dependency} for action {action}")]
