...
pub fn resolve(inputs: &[MaterialInput]) -> Result<Vec<Material>, ProceduralError> {
    let mut materials: BTreeMap<String, Material> = defaults().into_iter()
        .map(|material| (material.id.clone(), material)).collect();
    for input in inputs.iter().cloned() {
        if input.id.trim().is_empty() || input.base_color.iter().any(|v| !v.is_finite())
            || !input.roughness.is_finite() || !(0.0..=1.0).contains(&input.roughness)
            || !input.metallic.is_finite() || !(0.0..=1.0).contains(&input.metallic) {
...
pub fn defaults() -> Vec<Material> {
    vec![
        ...
        Material { id: "glass.window".to_owned(), base_color: [0.12, 0.23, 0.28, 0.72],
            roughness: 0.18, metallic: 0.0, double_sided: true, alpha_mode: AlphaMode::Blend },
        material("vegetation.trunk", [0.25, 0.14, 0.07, 1.0], 0.92, 0.0),
        Material { id: "vegetation.leaf".to_owned(), base_color: [0.12, 0.30, 0.10, 1.0],
            roughness: 0.94, metallic: 0.0, double_sided: true, alpha_mode: AlphaMode::Opaque },
    ]
}

fn material(id: &str, base_color: [f32; 4], roughness: f32, metallic: f32) -> Material {
    Material { id: id.to_owned(), base_color, roughness, metallic,
        double_sided: false, alpha_mode: AlphaMode::Opaque }
}
