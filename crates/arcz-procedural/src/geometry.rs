...
pub fn ensure_ccw(points: &[Point2]) -> Result<Vec<Point2>, ProceduralError> {
    validate_polygon(points, 0.01)
        .map_err(|error| ProceduralError::InvalidPolygon(error.to_string()))?;
    let mut output = points.to_vec();
    if signed_area(&output) < 0.0 {
        output.reverse();
    }
    Ok(output)
}
...
pub fn triangulate(points: &[Point2]) -> Result<Vec<[usize; 3]>, ProceduralError> {
    ...
    while remaining.len() > 3 {
        ...
        if iterations > maximum_iterations {
            return Err(ProceduralError::TriangulationFailed);
        }
        let mut ear = None;
        for i in 0..remaining.len() {
            ...
            if cross(points[a], points[b], points[c]) <= 1.0e-10 {
                continue;
            }
            if remaining.iter().copied().any(|p| {
                p != a
                    && p != b
                    && p != c
                    && point_in_triangle(points[p], points[a], points[b], points[c])
            }) {
                continue;
            }
            ear = Some((i, [a, b, c]));
...
pub fn axis_aligned_infill(
    polygon: &[Point2],
    front_setback: f64,
    side_setback: f64,
    maximum_coverage: f64,
) -> Option<Vec<Point2>> {
    if ensure_ccw(polygon).is_err() || !(0.05..=0.95).contains(&maximum_coverage) {
        return None;
    }
