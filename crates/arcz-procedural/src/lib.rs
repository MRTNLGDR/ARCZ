...
pub fn generate(
    ...
    seed: u64,
) -> Result<GeneratedArtifact, ProceduralError> {
    ...
    match kind {
        "terrain.generate" => {
            ...
            let mut scene = SceneOutput {
                materials,
                primitives: vec![terrain::generate(&grid)?],
                ..SceneOutput::default()
            };
            if parameters.terrain.is_none() {
                scene.warnings.push(
                    "terreno plano usado por autorização explícita; não representa DEM real"
                        .to_owned(),
                );
                scene.provenance.push(
                    json!({"entity":"terrain","estimated":true,"source":"explicit_flat_fallback"}),
                );
            }
            scene.validate()?;
        ...
        "parcels.generate" => {
            ...
            let mut scene = SceneOutput {
                materials,
                primitives: groups.into_primitives(),
                ..SceneOutput::default()
            };
        ...
        "houses.generate" | "buildings.generate" => {
            let requested = if kind == "houses.generate" {
        ...
        "vegetation.generate" => {
            let (batches, warnings, provenance) = greenery::generate(
                &parameters.vegetation_zones,
                parameters.quality,
                seed,
                parameters.vegetation_density_multiplier,
            )?;
            let mut scene = SceneOutput {
                materials,
        ...
        "materials.generate" => Ok(GeneratedArtifact {
            scene: None,
            json: Some(serde_json::to_value(materials)?),
        }),
        "tiles.generate" => {
            let input = parameters
                .tile_plan
                .ok_or(ProceduralError::InputMissing("tile_plan"))?;
            let tiles: Vec<PlannedTile> = plan_tiles(
                (input.focus[0], input.focus[1]),
                input.radius_m,
                input.zoom,
                input.rings_m,
                Seed(seed),
            )
            .map_err(|error| ProceduralError::Tile(error.to_string()))?;
            Ok(GeneratedArtifact {
                scene: None,
                json: Some(serde_json::to_value(tiles)?),
            })
        }
...
mod tests {
    ...
    fn terreno_real_gera_malha_validada() {
        let parameters = GeneratorParameters {
            terrain: Some(TerrainGrid {
                origin_enu_m: [0.0, 0.0],
                columns: 2,
                rows: 2,
                cell_size_m: [1.0, 1.0],
                heights_m: vec![0.0, 0.0, 0.0, 0.0],
                material_id: "terrain.grass".to_owned(),
            }),
    ...
    fn casa_explicita_gera_paredes_e_telhado() {
        let parameters = GeneratorParameters {
            buildings: vec![BuildingInput {
                id: "h1".to_owned(),
                footprint_enu_m: vec![[0.0, 0.0], [8.0, 0.0], [8.0, 6.0], [0.0, 6.0]],
                base_m: 0.0,
                height_m: 3.2,
                floors: 1,
                ...
                facade_module_width_m: 2.5,
                balcony_probability: 0.0,
                source: SourceEvidence::default(),
            }],
            ..GeneratorParameters::default()
        };
        let scene = generate("houses.generate", parameters, 2)
            .unwrap()
            .scene
            .unwrap();
    ...
    fn falta_de_dado_nao_vira_mock() {
        assert!(matches!(
            generate("terrain.generate", GeneratorParameters::default(), 0),
