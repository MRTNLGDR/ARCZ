...
use crate::input::{
    BuildingCategory, BuildingInput, EstimatedInfill, ParcelInput, Quality, RoofSpec,
    SourceEvidence,
};
...
pub fn generate(
    explicit: &[BuildingInput],
    parcels: &[ParcelInput],
    category: BuildingCategory,
    allow_estimated_infill: bool,
    infill: &EstimatedInfill,
    quality: Quality,
    seed: u64,
) -> Result<(MeshGroups, Vec<String>, Vec<serde_json::Value>), ProceduralError> {
    let mut buildings: Vec<BuildingInput> = explicit
        .iter()
        .filter(|item| category_matches(item.category, category))
        .cloned()
        .collect();
    ...
    if buildings.is_empty() && allow_estimated_infill {
        for (index, parcel) in parcels.iter().enumerate() {
            if let Some(footprint) = axis_aligned_infill(
                &parcel.polygon_enu_m,
                infill.front_setback_m,
                infill.side_setback_m,
                infill.maximum_coverage,
            ) {
                ...
                let building = BuildingInput {
                    id: format!("estimated:{}:{index}", parcel.id),
                    footprint_enu_m: footprint,
                    base_m: parcel.elevation_m,
                    height_m: height,
                    floors,
                    category: BuildingCategory::House,
                    roof: RoofSpec::default(),
                    wall_material_id: "facade.offwhite".to_owned(),
                    roof_material_id: "roof.ceramic".to_owned(),
                    glass_material_id: "glass.window".to_owned(),
                    balcony_material_id: "balcony.concrete".to_owned(),
                    commercial_ground_floor: false,
                    facade_module_width_m: 2.8,
                    balcony_probability: 0.12,
                    source: SourceEvidence {
                        source: "procedural_infill".to_owned(),
                        source_ref: parcel.id.clone(),
                        confidence: 0.35,
                        estimated: true,
                    },
                };
                ...
                warnings.push(format!(
                    "lote {} não comportou implantação estimada segura",
                    parcel.id
                ));
            }
        }
    ...
    for building in &buildings {
        generate_one(
            ...
            building,
            quality,
            Seed(seed).derive("building", &building.id),
            &mut warnings,
        )?;
...
fn generate_one(
    ...
    if !building.base_m.is_finite()
        || !building.height_m.is_finite()
        || building.height_m < 2.2
        || building.floors == 0
        || building.floors > 200
        || !(0.0..=1.0).contains(&building.balcony_probability)
    {
        return Err(ProceduralError::InvalidBuilding(building.id.clone()));
    }
    append_walls(
        groups.get_mut("building-walls", building.wall_material_id.clone()),
        &footprint,
        building.base_m,
        building.base_m + building.height_m,
    )?;
    let roof = generate_roof(&RoofRequest {
        footprint: footprint.clone(),
        wall_top_m: building.base_m + building.height_m,
        kind: building.roof.kind,
        pitch_deg: building.roof.pitch_deg,
        eave_m: building.roof.eave_m,
    })
    .map_err(|error| ProceduralError::Roof {
    ...
    warnings.extend(
        roof.warnings
            .into_iter()
            .map(|warning| format!("{}: {warning}", building.id)),
    );
    append_roof(
        groups.get_mut("building-roofs", building.roof_material_id.clone()),
        roof.mesh,
    )?;
    ...
    for edge_index in 0..footprint.len() {
        ...
        let facade = layout(
            &FacadeRequest {
                ...
                floor_height_m: floor_height.max(2.2),
                preferred_module_width_m: building.facade_module_width_m.clamp(1.2, 6.0),
                ground_floor_height_m: ground,
                balcony_probability: building.balcony_probability,
                commercial_ground_floor: building.commercial_ground_floor,
            },
            |_, _| rng.next_f64(),
        )
        .map_err(|error| ProceduralError::Facade {
            id: building.id.clone(),
            reason: error.to_string(),
        })?;
        for module in facade.modules {
            if module.kind == ModuleKind::Wall {
                continue;
            }
            ...
            let p0 = [
                a[0] + tangent[0] * x0 + outward[0] * offset,
                a[1] + tangent[1] * x0 + outward[1] * offset,
            ];
            let p1 = [
                a[0] + tangent[0] * x1 + outward[0] * offset,
                a[1] + tangent[1] * x1 + outward[1] * offset,
            ];
            let material = match module.kind {
                ModuleKind::Balcony if quality.balcony_detail() => {
                    building.balcony_material_id.clone()
                }
                _ => building.glass_material_id.clone(),
            };
            add_quad(
                groups.get_mut("facade-modules", material),
                render_point(p0, y0)?,
                render_point(p1, y0)?,
                render_point(p1, y1)?,
                render_point(p0, y1)?,
            ...
            if module.kind == ModuleKind::Balcony && quality.balcony_detail() {
                append_balcony(
                    groups.get_mut("balcony-slabs", building.balcony_material_id.clone()),
                    p0,
                    p1,
                    outward,
                    y0 - 0.12,
...
fn append_balcony(
    ...
    add_quad(
        primitive,
        render_point(p0, height)?,
        render_point(q0, height)?,
        render_point(q1, height)?,
        render_point(p1, height)?,
        [1.0, 1.0],
    )
}
