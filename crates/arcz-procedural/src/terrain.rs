...
pub fn from_flat(fallback: &FlatTerrainFallback) -> Result<TerrainGrid, ProceduralError> {
    ...
    if !(west.is_finite() && south.is_finite() && east.is_finite() && north.is_finite())
        || west >= east
        || south >= north
        || !fallback.elevation_m.is_finite()
    {
        return Err(ProceduralError::InvalidTerrain(
            "bounds/elevation inválidos".to_owned(),
        ));
    }
    ...
    let rows = resolution;
    Ok(TerrainGrid {
        origin_enu_m: [west, south],
        columns,
        rows,
        cell_size_m: [
            (east - west) / (columns - 1) as f64,
            (north - south) / (rows - 1) as f64,
        ],
...
pub fn generate(grid: &TerrainGrid) -> Result<Primitive, ProceduralError> {
    ...
    {
        return Err(ProceduralError::InvalidTerrain(
            "dimensões, espaçamento ou alturas inválidas".to_owned(),
        ));
    }
    let vertex_count = grid
        .columns
        .checked_mul(grid.rows)
        .ok_or(ProceduralError::GeometryTooLarge)?;
    if vertex_count > 16_777_216 {
        return Err(ProceduralError::GeometryTooLarge);
    }
    let mut primitive = Primitive {
        name: "terrain".to_owned(),
        material_id: grid.material_id.clone(),
    ...
    for row in 0..grid.rows - 1 {
        for column in 0..grid.columns - 1 {
            let a = u32::try_from(row * grid.columns + column)
                .map_err(|_| ProceduralError::GeometryTooLarge)?;
            let b = a + 1;
            let d = u32::try_from((row + 1) * grid.columns + column)
                .map_err(|_| ProceduralError::GeometryTooLarge)?;
            let c = d + 1;
...
fn normal_at(grid: &TerrainGrid, column: usize, row: usize) -> [f32; 3] {
    ...
    let dx_den = if column == 0 || column + 1 == grid.columns {
        grid.cell_size_m[0]
    } else {
        2.0 * grid.cell_size_m[0]
    };
    let dy_den = if row == 0 || row + 1 == grid.rows {
        grid.cell_size_m[1]
    } else {
        2.0 * grid.cell_size_m[1]
    };
