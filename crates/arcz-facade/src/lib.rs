...
fn default_ground() -> f64 { 4.0 }

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ModuleKind { Wall, Window, Balcony, Shopfront, Entrance, Service }
...
pub fn layout(request: &FacadeRequest, selector: impl Fn(u32, u32) -> f64) -> Result<FacadeLayout, FacadeError> {
    ...
    for floor in 0..request.floors {
        let base = if floor == 0 { 0.0 } else { request.ground_floor_height_m + (floor - 1) as f64 * request.floor_height_m };
        let height = if floor == 0 { request.ground_floor_height_m } else { request.floor_height_m };
        for column in 0..columns {
            let kind = if floor == 0 && request.commercial_ground_floor {
                if column == columns / 2 { ModuleKind::Entrance } else { ModuleKind::Shopfront }
            } else {
                let value = selector(floor, column).clamp(0.0, 1.0);
                if value < request.balcony_probability { ModuleKind::Balcony }
                else if value < 0.92 { ModuleKind::Window } else { ModuleKind::Wall }
            };
            modules.push(FacadeModule { floor, column, kind, x_m: column as f64 * module_width,
                                        base_m: base, width_m: module_width, height_m: height });
        }
    }
    Ok(FacadeLayout { columns, module_width_m: module_width, modules })
}
