...
pub enum WorldScope { Object, Parcel, Block, Neighborhood, City, State, Country, Continent, Planet }
...
#[serde(rename_all = "snake_case")]
pub enum CoordinateFrame { Wgs84, Ecef, LocalEnu }

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, PartialOrd, Ord)]
pub struct WorldCellId { pub level: u8, pub x: u64, pub y: u64 }

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum WorldLayerKind {
    Terrain, Imagery, Parcels, Buildings, Roads, Rail, Transit, Hydrology,
    Vegetation, Utilities, Atmosphere, Simulation, Authoring, Analysis,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum LayerMutability { ReadOnlySource, DerivedReadOnly, CanonicalEditable }
...
pub fn validate_world_project(project: &LockedWorldProject) -> Result<(), WorldContractError> {
    if project.project_id.trim().is_empty() { return Err(WorldContractError::MissingProjectId); }
    ...
    for layer in &project.layers {
        ...
        if layer.min_lod > layer.max_lod { return Err(WorldContractError::InvalidLodRange(layer.id.clone())); }
    ...
    if editable_authoring_layers > 1 { return Err(WorldContractError::MultipleCanonicalLayers); }
    Ok(())
}
...
pub enum WorldContractError {
    ...
    #[error("geographic anchor is outside WGS84 bounds")] InvalidAnchor,
    #[error("stream budget must be non-zero")] InvalidBudget,
    #[error("world layer id is empty or duplicated: {0}")] DuplicateOrEmptyLayer(String),
    #[error("invalid LOD range for layer {0}")] InvalidLodRange(String),
    #[error("only the ARCZ authoring layer may be canonical/editable: {0}")] EditableSourceLayer(String),
    #[error("a project may have at most one canonical editable authoring layer")] MultipleCanonicalLayers,
}
...
mod tests {
    ...
    fn rejects_editable_source_layers() {
        let project = LockedWorldProject {
            project_id: "p".into(), scope: WorldScope::City,
            anchor: GeoAnchor { latitude_deg: 0.0, longitude_deg: 0.0, altitude_m: 0.0, true_north_deg: 0.0 },
            frame: CoordinateFrame::LocalEnu, source_revision: 1, selected_cell_ids: vec![],
            layers: vec![WorldLayerPolicy {
                id: "terrain".into(), kind: WorldLayerKind::Terrain,
                mutability: LayerMutability::CanonicalEditable, min_lod: 0, max_lod: 18, provenance_required: true,
