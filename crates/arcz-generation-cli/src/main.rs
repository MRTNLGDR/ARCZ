...
fn run_main() -> Result<()> {
    ...
    let envelope: WorkerEnvelope =
        serde_json::from_slice(&raw).context("request envelope inválido")?;
    if envelope.schema_version != 1 {
        bail!("schema_version não suportado: {}", envelope.schema_version);
    }
    let root = PathBuf::from(&envelope.root)
        .canonicalize()
        .context("raiz ARCZ inválida")?;
    let output_dir = canonical_or_create(&output_dir)?;
    output_dir
        .strip_prefix(&root)
        .context("output-dir precisa estar dentro da raiz ARCZ")?;
    let declared_staging = canonical_or_create(Path::new(&envelope.staging_dir))?;
    if output_dir != declared_staging {
        bail!("output-dir diverge de staging_dir declarado");
    }
    progress(
        &output_dir,
        "VALIDATE_REQUEST",
    ...
    let plugin_id = envelope
        ...
        .unwrap_or("arcz.worker");
    let plugin_version = envelope
        .request
        .get("plugin_version")
        .and_then(Value::as_str)
        .unwrap_or("0.0.0");
    let request_hash = sha256_hex(canonical_json(&envelope.request));
    let profile_value = envelope
        .request
        .pointer("/region/profile")
        .cloned()
        .or_else(|| envelope.request.pointer("/region/context").cloned())
        .unwrap_or_else(|| json!({}));
    ...
    progress(
        ...
        "ACQUIRE_INPUTS",
        0.10,
        "Resolvendo somente entradas materializadas",
        None,
    )?;
    let artifact = if envelope.kind == "region.context.generate" {
        let context = envelope
            .request
            .pointer("/region/context")
            .cloned()
        ...
        progress(
            ...
            "GENERATE",
            0.20,
            "Executando gramática procedural local",
            None,
        )?;
    ...
    progress(
        ...
        "VALIDATE_OUTPUT",
        0.78,
        "Validando malhas e preparando artefatos",
        None,
    )?;
    ...
    if let Some(scene) = artifact.scene.as_ref() {
        ...
        metrics.insert(
            "estimated_resources".to_owned(),
            serde_json::to_value(scene.estimated_resources())?,
        );
    }
    ...
    if outputs.is_empty() {
        bail!("worker terminou sem artefatos");
    }
    ...
    progress(
        ...
        "PERSIST",
        0.94,
        "Gravando manifest e checksums",
        Some(json!({"outputs":outputs.len()})),
    )?;
...
fn parse_args() -> Result<(PathBuf, PathBuf)> {
    ...
    if args.next().as_deref() != Some("run") {
        bail!("uso: arcz-generation-cli run --request <json> --output-dir <dir>");
    }
    ...
    Ok((
        request.context("--request ausente")?,
        output.context("--output-dir ausente")?,
    ))
}
...
fn flattened_parameters(request: &Value) -> Result<Value> {
    ...
    let object = params
        .as_object_mut()
        .context("request.params precisa ser objeto")?;
    if let Some(inputs) = object.remove("inputs") {
        let inputs = inputs
            .as_object()
            .context("params.inputs precisa ser objeto")?;
        for (key, value) in inputs {
            if object.contains_key(key) {
                bail!("campo duplicado em params e params.inputs: {key}");
            }
...
fn explicit_or_derived_seed(request: &Value, request_hash: &str) -> Result<u64> {
    if let Some(value) = request.pointer("/params/seed") {
        return value
            .as_u64()
            .context("params.seed precisa ser inteiro sem sinal");
    }
    let raw = u64::from_str_radix(&request_hash[..16], 16).context("derivar seed")?;
    Ok(Seed(raw)
        .derive("arcz-generation", request_hash.as_bytes())
        .0)
}
...
fn source_versions(request: &Value) -> Value {
    ...
    if let Some(values) = request
        .pointer("/region/context/source_packages")
        .and_then(Value::as_array)
    {
        for value in values.iter().filter_map(Value::as_str) {
            result.insert(value.to_owned(), json!("materialized"));
        }
    }
    Value::Object(result)
}
...
fn canonical_or_create(path: &Path) -> Result<PathBuf> {
    ...
    path.canonicalize()
        .with_context(|| format!("canonicalizar {}", path.display()))
}
...
fn write_json(path: &Path, value: &Value) -> Result<()> {
    ...
    ));
    {
        let mut file = fs::File::create(&temporary)
            .with_context(|| format!("criar {}", temporary.display()))?;
