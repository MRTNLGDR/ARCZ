...
fn default_attempts() -> usize { 40 }

#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
pub struct ExclusionCircle { pub center: Point2, pub radius_m: f64 }
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VariantWeight { pub id: String, pub weight: f64, pub scale_min: f64, pub scale_max: f64 }
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Instance { pub position: Point2, pub rotation_rad: f64, pub scale: f64, pub variant: String, pub reason: String }

pub fn scatter(request: &ScatterRequest) -> Result<Vec<Instance>, VegetationError> {
    validate_polygon(&request.polygon, 0.25).map_err(|e|VegetationError::InvalidPolygon(e.to_string()))?;
    if request.target_count > 1_000_000 || !request.minimum_distance_m.is_finite() || request.minimum_distance_m < 0.0 || request.attempts_per_instance == 0 {
        return Err(VegetationError::InvalidParameter);
    }
    let min_x=request.polygon.iter().map(|p|p[0]).fold(f64::INFINITY,f64::min);let max_x=request.polygon.iter().map(|p|p[0]).fold(f64::NEG_INFINITY,f64::max);
    let min_y=request.polygon.iter().map(|p|p[1]).fold(f64::INFINITY,f64::min);let max_y=request.polygon.iter().map(|p|p[1]).fold(f64::NEG_INFINITY,f64::max);
...
fn distance(a:Point2,b:Point2)->f64{(a[0]-b[0]).hypot(a[1]-b[1])}
fn point_in_polygon(p: Point2, poly: &[Point2]) -> bool {
    ...
    for i in 0..poly.len() {
        ...
        if crosses_scanline {
            ...
            if p[0] < x_intersection { inside = !inside; }
        }
        j = i;
    }
    inside
...
#[derive(Debug,Error)]pub enum VegetationError{#[error("polígono inválido: {0}")]InvalidPolygon(String),#[error("parâmetro de scatter inválido")]InvalidParameter}
#[cfg(test)]mod tests{use super::*;#[test]fn scatter_reproduz(){let r=ScatterRequest{polygon:vec![[0.,0.],[10.,0.],[10.,10.],[0.,10.]],seed:1,target_count:10,minimum_distance_m:1.,exclusions:vec![],attempts_per_instance:40,variants:vec![]};assert_eq!(scatter(&r).unwrap().iter().map(|x|x.position).collect::<Vec<_>>(),scatter(&r).unwrap().iter().map(|x|x.position).collect::<Vec<_>>());}}
