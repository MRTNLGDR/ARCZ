...
pub struct PermissionGrant {
    #[serde(default)] pub capabilities: BTreeSet<String>,
    #[serde(default)] pub readable_domains: BTreeSet<String>,
    #[serde(default)] pub writable_domains: BTreeSet<String>,
}
...
impl PluginRegistry {
    pub fn register(&mut self, manifest: PluginManifest) -> Result<(), PluginHostError> {
        ...
        if self.manifests.contains_key(&manifest.id) { return Err(PluginHostError::DuplicatePlugin(manifest.id)); }
    ...
    pub fn get(&self, id: &str) -> Option<&PluginManifest> { self.manifests.get(id) }

    pub fn providers_for(&self, capability: &str) -> Vec<&PluginManifest> {
        self.capability_index.get(capability).into_iter().flatten().filter_map(|id| self.manifests.get(id)).collect()
    }

    pub fn authorize(&self, plugin_id: &str, requested_capability: &str, grant: &PermissionGrant) -> Result<(), PluginHostError> {
        ...
        if !plugin.capabilities.iter().any(|cap| cap.0 == requested_capability) {
            return Err(PluginHostError::CapabilityNotDeclared { plugin: plugin_id.into(), capability: requested_capability.into() });
        }
        if !grant.capabilities.contains(requested_capability) {
            return Err(PluginHostError::CapabilityNotGranted { plugin: plugin_id.into(), capability: requested_capability.into() });
        }
        Ok(())
    }
...
pub enum PluginHostError {
    #[error(transparent)] InvalidManifest(#[from] PluginContractError),
    #[error("plugin already registered: {0}")] DuplicatePlugin(String),
    #[error("unknown plugin: {0}")] UnknownPlugin(String),
    #[error("plugin {plugin} did not declare capability {capability}")] CapabilityNotDeclared { plugin: String, capability: String },
    #[error("plugin {plugin} was not granted capability {capability}")] CapabilityNotGranted { plugin: String, capability: String },
}
