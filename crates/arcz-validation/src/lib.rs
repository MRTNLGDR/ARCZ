...
pub fn validate_polygon(polygon: &[Point2], minimum_area: f64) -> Result<(), GeometryError> {
    if polygon.len() < 3 { return Err(GeometryError::TooFewVertices); }
    for point in polygon {
        if !point[0].is_finite() || !point[1].is_finite() { return Err(GeometryError::NonFinite); }
    }
    if signed_area(polygon).abs() < minimum_area { return Err(GeometryError::AreaTooSmall); }
    let n = polygon.len();
    for i in 0..n {
        let a = polygon[i]; let b = polygon[(i + 1) % n];
        for j in i + 1..n {
            let c = polygon[j]; let d = polygon[(j + 1) % n];
            if i == j || (i + 1) % n == j || (j + 1) % n == i { continue; }
            if segments_intersect(a, b, c, d) { return Err(GeometryError::SelfIntersection { a: i, b: j }); }
        }
    }
    Ok(())
}
...
pub fn validate_mesh(mesh: MeshView<'_>) -> Result<ValidationReport, GeometryError> {
    if mesh.positions.is_empty() { return Err(GeometryError::EmptyMesh); }
    if mesh.normals.len() != mesh.positions.len() || mesh.uvs.len() != mesh.positions.len() {
        return Err(GeometryError::AttributeLengthMismatch);
    }
    if mesh.indices.len() % 3 != 0 { return Err(GeometryError::IndexCountNotTriangular); }
    ...
    for (index, position) in mesh.positions.iter().enumerate() {
        for axis in 0..3 {
            ...
            min[axis] = min[axis].min(position[axis]); max[axis] = max[axis].max(position[axis]);
        }
        if !mesh.uvs[index][0].is_finite() || !mesh.uvs[index][1].is_finite() { return Err(GeometryError::NonFinite); }
    }
    let mut warnings = Vec::new();
    for (triangle, indices) in mesh.indices.chunks_exact(3).enumerate() {
        if indices.iter().any(|index| *index as usize >= mesh.positions.len()) {
            return Err(GeometryError::IndexOutOfBounds { triangle });
        }
        let a = mesh.positions[indices[0] as usize]; let b = mesh.positions[indices[1] as usize]; let c = mesh.positions[indices[2] as usize];
        ...
        if area2 < 1.0e-8 { warnings.push(format!("triângulo degenerado: {triangle}")); }
    }
    Ok(ValidationReport { warnings, triangles: mesh.indices.len()/3, bbox_min: min, bbox_max: max })
}
...
mod tests {
    ...
    #[test]
    fn detecta_bow_tie() {
        assert!(matches!(validate_polygon(&[[0.,0.],[1.,1.],[0.,1.],[1.,0.]], 0.0), Err(GeometryError::SelfIntersection{..}) | Err(GeometryError::AreaTooSmall)));
    }
}
