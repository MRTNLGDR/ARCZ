...
//!
//! The first stable boundary is data-oriented on purpose: built-in Rust crates,
//! WASM components, local processes and web sidecars all describe themselves with
//! the same manifest and capability model. A plugin never owns the canonical scene.
...
pub fn validate_manifest(manifest: &PluginManifest) -> Result<(), PluginContractError> {
    ...
    if manifest.api_version != PLUGIN_API_VERSION {
        return Err(PluginContractError::UnsupportedApi(manifest.api_version));
    }
    if manifest.entrypoint.trim().is_empty() {
        return Err(PluginContractError::MissingEntrypoint);
    }
    let mut seen = BTreeSet::new();
    for capability in &manifest.capabilities {
        let value = capability.0.trim();
        if value.is_empty() {
            return Err(PluginContractError::EmptyCapability);
        }
        if !seen.insert(value.to_owned()) {
            return Err(PluginContractError::DuplicateCapability(value.to_owned()));
        }
    }
...
pub enum PluginContractError {
    #[error("plugin identity fields are required")]
    MissingIdentity,
    #[error("unsupported plugin API version {0}")]
    UnsupportedApi(u32),
    #[error("plugin entrypoint is required")]
    MissingEntrypoint,
    #[error("plugin capability must not be empty")]
    EmptyCapability,
    #[error("duplicate plugin capability {0}")]
    DuplicateCapability(String),
    #[error("explicit remote network requires network.remote capability")]
    RemoteNetworkWithoutCapability,
}
