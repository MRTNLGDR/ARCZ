...
```text
ARCZ UI (Web/WASM/WebGPU + Desktop/Tauri)
             │ typed commands/events
             ▼
ARCZ Authoring Kernel — scene · CAD · BIM · revisions
             │
      ┌──────┴────────┐
      ▼               ▼
Plugin Host/SDK   ARCZ World Authority
capability ACL    cells · layers · LOD · budgets
      │               │
      └──────┬────────┘
             ▼
procedural · AI workers · GIS · render · simulation
             │
             ▼
GLB/glTF · IFC · 3D Tiles · sheets · renders
```

The globe renderer is never the canonical editable scene.
...
- specialized generator/worker adapters for terrain, buildings, roads, vegetation, IFC, assets, render and simulation.
